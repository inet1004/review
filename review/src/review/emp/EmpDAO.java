...
public class EmpDAO { 
	...
	// 사원 단건 조회
	public EmpDTO getEmp(int gb_id) {  //단건 조회
		conn = ConnectionManager.getConnnection();		
	...
	public ArrayList<EmpDTO> getDBList(EmpSearchDTO dto) {
		...
		String sql ="select a.* from ( select rownum rn, b.* from ( " +
					"select * from employees " + where + " order by EMPLOYEE_ID desc" +
	...
	public List<Map<String, Object>> getRegion(){
		...
		try {
			conn = ConnectionManager.getConnnection();
			String sql = "select l.city, c.country_id, l.country_id, country_name, region_name \r\n" + 
			...
			while(rs.next()) {
				...
				map.put("city", rs.getString("country_id"));
				map.put("city", rs.getString("country_name"));
				map.put("city", rs.getString("region_name"));
				list.add(map);
			}
		} catch (Exception e) {
			e.printStackTrace();
